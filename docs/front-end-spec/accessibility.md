...
**MVP Goal:** Reasonable accessibility practices with keyboard navigation and
screen reader support for primary actions. Formal WCAG 2.1 AA compliance is
deferred to Phase 2 to maintain MVP timeline, but we'll implement fundamental
accessibility patterns that support most users.
...
- **Tab:** Navigate through add task input → add button → first task → task
  actions → next task
...
- **Focus management:** When modal opens, focus moves to first interactive
  element; when closed, focus returns to trigger element
...
- Use appropriate HTML elements: `<button>`, `<input>`, `<label>`,
  `<h1>`-`<h6>`, `<ul>`, `<li>`
...
**Note:** Toast buttons include task-specific aria-labels so screen reader users
navigating by button have full context, even if they skip the prompt text.
...
- **Visibility:** Never remove focus indicators with `outline: none` without
  providing alternative

**Focus Order:**

- Logical tab order: follows visual layout (top to bottom, left to right)
- Skip links (optional enhancement): "Skip to main content" link for keyboard
  users
...
- All interactive elements: **44x44px minimum** (WCAG 2.1 AA Success Criterion
  2.5.5)
...
- Users who enable "Reduce Motion" in OS settings see instant transitions
...
<div>
  ...
  <input id="task-input" aria-invalid="true" aria-describedby="task-error" />
...
**Rationale:** MVP focuses on fundamental accessibility (keyboard nav, screen
readers, semantic HTML) to validate core product hypotheses within 4-6 week
timeline. Phase 2 will conduct formal accessibility audit and remediation.
